...
- 评估与推理
  ...
  - **新增**：自动设备检测与GPU兼容性检查，支持V100 GPU（计算能力7.0）的智能降级
- MICA 多模态融合模型
...
- **增强的推理流程**（test_codex_lung_marker.py）
  - **自动设备检测**：智能检测可用设备，优先使用CUDA，否则回退到CPU
  - **GPU兼容性检查**：特别针对V100 GPU（计算能力7.0）进行兼容性检查，自动降级到CPU模式
  - **混合精度推理**：使用autocast进行半精度推理，显著提升推理速度
  - **动态内存管理**：根据设备类型动态设置pin_memory参数
  - 保存 patch 级别预测与标签，计算每生物标志物的皮尔逊相关系数
- 训련流程（train_dist_codex_lung_marker.py）
  - 分布式训练，DDP 包装模型
...
DeviceManager --> CustomModel : "智能设备管理"
```
...
- [hex/test_codex_lung_marker.py:62-82](file://hex/test_codex_lung_marker.py#L62-L82)
...
- **增强的设备管理**
  - **自动设备检测**：智能选择CUDA或CPU设备，提升兼容性
  - **GPU兼容性检查**：针对V100 GPU（CC 7.0）的特殊处理，确保稳定运行
  - **动态内存优化**：根据设备类型调整pin_memory参数
- 回归稳定性
...
- [hex/test_codex_lung_marker.py:68-76](file://hex/test_codex_lung_marker.py#L68-L76)
...
- **设备兼容性问题**
  - **V100 GPU（CC 7.0）**：系统会自动检测并降级到CPU模式，无需手动干预
  - **CUDA版本不匹配**：检查CUDA绑定版本与PyTorch版本兼容性
  - **设备能力检测失败**：确认GPU驱动程序正常安装
- 指标异常
  - 检查标签范围与归一化是否一致
...
## 结论
本项目提供了从 H&E 切片到虚拟 CODEX 表达的完整回归管线，并进一步将虚拟 CODEX 与 H&E 片段 bag 融合，构建生存分析模型。通过分布式训练、FDS 平滑与稳健损失、5 折交叉验证与 c-index 指标，系统在可解释性与泛化性能之间取得平衡。

**最新改进**：增强了模型加载机制，包括自动设备检测、GPU兼容性检查（特别是V100 CC 7.0兼容性）和混合精度推理支持，显著提升了系统的稳定性和兼容性。

建议在实际部署中结合 A/B 测试与长期监控，持续评估模型在不同人群与中心的稳定性。
...
- **设备兼容性说明**
  - 系统会自动检测可用设备并进行兼容性检查
  - V100 GPU（计算能力7.0）将自动降级到CPU模式以确保稳定运行
...
### 设备兼容性检查机制
**新增功能**：系统内置了智能的设备兼容性检查机制，特别针对V100 GPU（计算能力7.0）进行了专门优化。

- **自动设备检测**
  - 优先检测CUDA可用性
  - 如果CUDA不可用，则回退到CPU模式
- **GPU兼容性检查**
  - 检测GPU计算能力（compute capability）
  - V100 GPU（CC 7.0）会被识别并自动降级到CPU模式
  - 其他GPU设备保持正常CUDA推理
- **混合精度支持**
  - CUDA设备使用半精度（float16）推理
  - CPU设备使用全精度（float32）推理
- **内存优化**
  - CUDA设备启用pin_memory以提升数据传输效率
  - CPU设备禁用pin_memory以节省内存

**Section sources**
- [hex/test_codex_lung_marker.py:68-76](file://hex/test_codex_lung_marker.py#L68-L76)
- [hex/test_codex_lung_marker.py:131-132](file://hex/test_codex_lung_marker.py#L131-L132)
- [hex/test_codex_lung_marker.py:123](file://hex/test_codex_lung_marker.py#L123)